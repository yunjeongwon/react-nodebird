...
const Profile = () => {
  ...
  const { data: followersData, error: followerError } = useSWR(`http://nodebirdjw.shop/user/followers?limit=${followersLimit}`, fetcher);
  const { data: followingsData, error: followingError } = useSWR(`http://nodebirdjw.shop/user/followings?limit=${followingsLimit}`, fetcher);
  ...
  if (followerError || followingError) {
    ...
    return '팔로윙/팔로워 로딩 중 에러가 발생했습니다.';
  }
